...
export const ReadmeEditor = () => {
  ...
  const generateReadme = async () => {
    if (!formData.projectName || !formData.description) {
      setErrors({ projectName: !formData.projectName, description: !formData.description });
      return;
    }
    ...
    try {
      ...
      const prompt = `
        ...
        - Inclua badges do shields.io dinâmicos (repo size, license, stars).
        - Use emojis apropriados.
        - Seção de instalação com blocos de código formatados.
        - Retorne apenas o código Markdown.
      `;
      ...
      console.error(error);
      alert("Houve um problema ao gerar seu README. Verifique sua conexão e chave de API.");
    } finally {
        ...
        <div className="lg:col-span-5 space-y-8">
          <div className="bg-zinc-900/40 border border-zinc-800/50 rounded-[2.5rem] p-10 space-y-10 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between relative z-10">
              ...
              <button onClick={() => setFormData({ ...formData, projectName: '', description: '' })} className="text-zinc-600 hover:text-white transition-colors" title="Limpar">
            ...
            <div className="space-y-8 relative z-10">
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Nome do Projeto</label>
                  <input 
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Tom de Voz</label>
                  <select 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Descrição do Produto</label>
                <textarea 
                  name="description" value={formData.description} onChange={handleInputChange} 
                  rows={4} className={`w-full bg-black/60 border ${errors.description ? 'border-red-500/50' : 'border-zinc-800'} rounded-2xl px-5 py-4 text-sm focus:border-indigo-500 outline-none resize-none transition-all`}
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Tech Stack</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Template de Design</label>
                  <select 
                    name="template" value={formData.template} onChange={handleInputChange}
                    className="w-full bg-black/60 border border-zinc-800 rounded-2xl px-5 py-4 text-sm font-bold outline-none focus:border-indigo-500"
                  >
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Comando de Instalação</label>
                <div className="relative">
                  <input 
                    name="installation" value={formData.installation} onChange={handleInputChange} 
              ...
              <button 
                onClick={generateReadme} disabled={isGenerating}
          ...
          <div className="bg-indigo-500/5 border border-indigo-500/10 rounded-3xl p-6 flex items-center justify-between group cursor-help">
             <div className="flex items-center gap-4">
                ...
                <div>
                   <p className="text-xs font-bold text-zinc-300">Privacidade Garantida</p>
                   <p className="text-[10px] text-zinc-500">Seus dados nunca são armazenados.</p>
                </div>
             </div>
          ...
          <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-[2.5rem] flex flex-col min-h-[750px] max-h-[85vh] shadow-2xl relative overflow-hidden">
              ...
              {markdown ? (
                ...
                <div className="h-full flex flex-col items-center justify-center opacity-10 space-y-8 select-none">
                  ...
                  <div className="text-center space-y-2">
                    <p className="text-2xl font-black uppercase tracking-widest">Workspace Vazio</p>
                    <p className="text-xs max-w-xs leading-relaxed font-bold uppercase tracking-tighter">Preencha o formulário e deixe a mágica acontecer.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            {markdown && (
              <div className="px-10 py-4 bg-zinc-950 border-t border-zinc-800/50 flex items-center justify-between">
                 <div className="flex items-center gap-3">
                    <span className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">MD Engine v2.0</span>
                    <div className="h-4 w-px bg-zinc-900" />
                    <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">Otimizado</span>
                 </div>
              </div>
            )}
          </div>
